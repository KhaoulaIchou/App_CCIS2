...
public class LieuEE extends Application implements Initializable {
    ...
    public void Trier() {
        ...
        try {
            ...
            String s1 = "Select COUNT(*) AS count FROM EE WHERE LOWER(Lieu)= 'essaouira' ";
            ResultSet rst1 = stm.executeQuery(s1);
            rst1.next();
            int nb = rst1.getInt("count");


            String s2 = "Select COUNT(*) AS count2 FROM EE WHERE LOWER(Lieu)= 'marrakech' ";
            ResultSet rst2 = stm.executeQuery(s2);
            rst2.next();
            int nb2 = rst2.getInt("count2");


            String s3 = "Select COUNT(*) AS count3 FROM EE WHERE LOWER(Lieu)= 'safi' ";
            ResultSet rst3 = stm.executeQuery(s3);
            rst3.next();
            int nb3 = rst3.getInt("count3");


            String s4 = "Select COUNT(*) AS count4 FROM EE WHERE Lieu LIKE 'EL KELAA%' ";
            ...
            data.addAll(
                    new PieChart.Data("Essaouira", p1),
                    new PieChart.Data("Marrakech", p2),
                    new PieChart.Data("Safi", p3),
                    new PieChart.Data("EL Kelaa Des Sraghna", p4));
            pieChart.setData(data);
            for(final PieChart.Data data: pieChart.getData()){
                data.nameProperty().set(data.getName()+"\n"+(float)data.getPieValue()+"%");
            }
           /* String S1="SELECT COUNT(*) AS count, Lieu FROM Dema GROUP BY Lieu";
            ...
            while(rst1.next()){
                counts[i]=rst1.getInt("count");
                Names[i]=rst1.getString("Lieu");
            }
            float pourcentage[]=new float[index];
            for (i=0;i<index;i++){
                pourcentage[i]=(float) 100 * counts[i] / totalVille;
            }for (i=0;i<index;i++){
                pourcentage[i]=(float) 100 * counts[i] / totalVille;
            }for (i=0;i<index;i++){
                data.addAll( new PieChart.Data(Names[i], counts[i]));
            }
            pieChart.setData(data);*/
            stm.close();
            con.close();
