...
/*************************************Statistiques du document demandé - DA *********************************************************/
public class Document extends Application implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        //un tableau trié selon le document demandé
        ...
        try {
            ...
            String s1 = "Select COUNT(*) AS count FROM Dema WHERE Document LIKE'Carte%' ";
            ResultSet rst1 = stm.executeQuery(s1);
            rst1.next();
            int nb = rst1.getInt("count");
            ;

            String s2 = "Select COUNT(*) AS count2 FROM Dema WHERE Document LIKE 'Attestation%' ";
            ResultSet rst2 = stm.executeQuery(s2);
            rst2.next();
            int nb2 = rst2.getInt("count2");


            String s3 = "Select COUNT(*) AS count3 FROM Dema WHERE Document LIKE '%origine%'";
            ResultSet rst3 = stm.executeQuery(s3);
            rst3.next();
            int nb3 = rst3.getInt("count3");


            String s4 = "Select COUNT(*) AS count4 FROM Dema WHERE LOWER(Document)= 'visa des factures' ";
            ResultSet rst4 = stm.executeQuery(s4);
            rst4.next();
            int nb4 = rst4.getInt("count4");

            String s5 = "Select COUNT(*) AS count5 FROM Dema WHERE LOWER(Document)= 'certificat de vente libre' ";
            ResultSet rst5 = stm.executeQuery(s5);
            rst5.next();
            int nb5 = rst5.getInt("count5");

            String s6 = "Select COUNT(*) AS count6 FROM Dema WHERE LOWER(Document)= 'visa des documents commerciaux' ";
            ResultSet rst6 = stm.executeQuery(s6);
            rst6.next();
            int nb6 = rst6.getInt("count6");

            String s7 = "Select COUNT(*) AS count7 FROM Dema WHERE Document LIKE '%sanitaires%' ";
            ResultSet rst7 = stm.executeQuery(s7);
            rst7.next();
            int nb7 = rst7.getInt("count7");

            String s8 = "Select COUNT(*) AS count8 FROM Dema WHERE LOWER(Document) = 'recommandation pour visa affaires' ";
            ResultSet rst8 = stm.executeQuery(s8);
            rst8.next();
            int nb8 = rst8.getInt("count8");

            String s9 = "Select COUNT(*) AS count9 FROM Dema WHERE LOWER(Document) = 'autre' ";
            ResultSet rst9 = stm.executeQuery(s8);
            rst9.next();
            int nb9 = rst9.getInt("count8");

            float p1 = (float) 100 * nb / totalVille;
            float p2 = (float) 100 * nb2 / totalVille;
            float p3 = (float) 100 * nb3 / totalVille;
            float p4 = (float) 100 * nb4 / totalVille;
            float p5 = (float) 100 * nb5 / totalVille;
            float p6 = (float) 100 * nb6 / totalVille;
            float p7 = (float) 100 * nb7 / totalVille;
            float p8 = (float) 100 * nb8 / totalVille;
            float p9 = (float) 100 * nb9 / totalVille;
             p1 = (float)Math.round(p1*100)/100;
             p2 = (float)Math.round(p2*100)/100;
             p3 = (float)Math.round(p3*100)/100;
             p4= (float)Math.round(p4*100)/100;
             p5 = (float)Math.round(p5*100)/100;
             p6 = (float)Math.round(p6*100)/100;
             p7 = (float)Math.round(p7*100)/100;
             p8= (float)Math.round(p8*100)/100;
             p9= (float)Math.round(p9*100)/100;

            data.addAll(
                    new PieChart.Data("Carte professionnelle", p1),
                    new PieChart.Data("Attestation professionnelle", p2),
                    new PieChart.Data("Certificat d'origine", p3),
                    new PieChart.Data("Visa des factures", p4),
                    new PieChart.Data("Certificat de vente libre", p5),
                    new PieChart.Data("Visa des documents commerciaux", p6),
                    new PieChart.Data("Visa de certificats sanitaires/phytosanitaires", p7),
                    new PieChart.Data("Recommandation pour Visa Affaires", p8),
                    new PieChart.Data("Autre", p9)
            );
    ...
    //supprimer une ligne spécifique du tableau Excel
    ...
    public void addAction(ActionEvent actionEvent) {
        try {
            FXMLLoader fxmlLoader = new FXMLLoader(getClass().getResource("Demarche.fxml")); //charger la fiche DA
