...
public class LieuGuichet extends Application implements Initializable {
    @FXML
    ...
    public void Trier() {
        ...
        try {
            ...
            String s1 = "Select COUNT(*) AS count FROM Guichet WHERE LOWER(Lieu)= 'essaouira' ";
            ResultSet rst1 = stm.executeQuery(s1);
            rst1.next();
            int nb = rst1.getInt("count");


            String s2 = "Select COUNT(*) AS count2 FROM Guichet WHERE LOWER(Lieu)= 'marrakech' ";
            ResultSet rst2 = stm.executeQuery(s2);
            rst2.next();
            int nb2 = rst2.getInt("count2");


            String s3 = "Select COUNT(*) AS count3 FROM Guichet WHERE LOWER(Lieu)= 'safi' ";
            ResultSet rst3 = stm.executeQuery(s3);
            rst3.next();
            int nb3 = rst3.getInt("count3");


            String s4 = "Select COUNT(*) AS count4 FROM Guichet WHERE Lieu LIKE 'EL KELAA%' ";
            ResultSet rst4 = stm.executeQuery(s4);
            rst4.next();
            int nb4 = rst4.getInt("count4");


            float pEss = (float) 100 * nb / totalVille;
            float pMa = (float) 100 * nb2 / totalVille;
            float pSa = (float) 100 * nb3 / totalVille;
            float pEl = (float) 100 * nb4 / totalVille;
            float p1 = (float)Math.round(pEss*100)/100;
            float p2 = (float)Math.round(pMa*100)/100;
            float p3 = (float)Math.round(pSa*100)/100;
            float p4= (float)Math.round(pEl*100)/100;
            data.addAll(
                    new PieChart.Data("Essaouira", pEss),
                    new PieChart.Data("Marrakech", pMa),
                    new PieChart.Data("Safi", pSa),
                    new PieChart.Data("EL Kelaa Des Sraghna", pEl));
